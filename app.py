...
def get_ai_msg(df, user_msg=""):
    ...
    try:
        ...
        if user_msg.strip():
            prompt = f"User: {user_msg}\nGive 1 sentence motivation. Max 10 words!"
        else:
            prompt = f"Gym: {gym}/7, Study: {study:.1f}h\nGive 1 sentence power message. Max 10 words!"
...
<div class="header">
    ...
    <p class="subtitle">>> Build Habits. Track Progress</p>
</div>
""", unsafe_allow_html=True)
...
with st.container():
    ...
    col_chat, col_btn = st.columns([1, 0.12], gap="small")
    with col_chat:
        user_input = st.text_input("", placeholder="Ask...", label_visibility="collapsed", key="chat")
    with col_btn:
        if st.button("→", key="send"):
            if user_input.strip():
                resp = get_ai_msg(df, user_input)
                st.markdown(f'<div class="ai-message" style="color: #00ff99;">🤖: {resp}</div>', unsafe_allow_html=True)
...
st.markdown("""
<div class="footer">
>> Stay Focused. Keep Grinding. Dominate 🚀
</div>
""", unsafe_allow_html=True)
